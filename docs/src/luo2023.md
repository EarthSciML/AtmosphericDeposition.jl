...
The cloud ice uptake rate ``R_U`` for HNO₃ (Eq. 14, based on Jacob, 2000) is:

```math
R_{U,\text{HNO}_3} = \frac{N_I \cdot S_I}{\frac{r}{D_g} + \frac{2.749064 \times 10^{-4}\,\sqrt{M}}{\gamma\,\sqrt{T}}}
```

where ``N_I`` is ice crystal number concentration, ``S_I`` is ice crystal surface area, ``r`` is ice crystal radius, ``D_g`` is gas-phase diffusion coefficient, ``M`` is molar mass, ``T`` is temperature, and ``\gamma`` is uptake efficiency. The uptake efficiency for HNO₃ on ice (Eq. 15, based on Hudson et al., 2002) is:
...
| Pressure | HNO₃ (AR) | InOrgNIT (AR) | NH₄ (AR) | SO₄ (AR) | HNO₃ (ARIU) | InOrgNIT (ARIU) | NH₄ (ARIU) | SO₄ (ARIU) |
|----------|-----------|--------------|---------|---------|------------|----------------|-----------|-----------|
