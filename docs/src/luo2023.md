...
```@example luo2023
using DataFrames, ModelingToolkit, DynamicQuantities, Symbolics
...
for tke_val in tke_vals_plot
    ...
    for dx_val in dx_range
        ...
        R_A_val = to_float(substitute(R_A_expr2, vals))
...
γ_vals = [to_float(substitute(γ_expr, merge(const_defaults, Dict(T_plot => T_val))))
...
for dt_val in dt_vals
    F_I_vals = [to_float(substitute(F_I_expr, Dict(R_AU_plot => rau, Δt_plot => dt_val)))
